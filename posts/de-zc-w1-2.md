...
The concept of adding an application to a system path may be simple to some, but for me it was longer than I'd like to admit before I realized what this actually did. Before we can run any Terraform commands, we must authenticate to GCP with our Service Account. We do this by running the command `gcloud auth application-default login` in the Google Cloud SDK terminal. Now if you were to run that same command in the Windows CLI, you would notice that will not work. Well, at least not immediately. Why is that? That is because we haven't made our system aware of where the Google Cloud SDK is located on our system. This is what adding gcloud to our system path does. 
